codeunit 50101 "Customer Rewards Ext. Mgt."
{
        ...
        if (GetHttpResponse(ActivationCode, ResponseText)) then begin
            ...
            if (JsonRepsonse.SelectToken('ActivationResponse', Result)) then
                if (Result.AsValue().AsText() = 'Success') then begin
                    ...
                    ActivationCodeInfo.ActivationCode := CopyStr(ActivationCode, 0, 14);
    ...
    begin
        if CustomerRewardsExtMgtSetup.Get() then
            exit(CustomerRewardsExtMgtSetup."Customer Rewards Ext. Mgt. ID" = CODEUNIT::"Customer Rewards Ext. Mgt.");
        exit(false);
    end;
}